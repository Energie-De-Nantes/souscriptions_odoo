...
class TestRaccordementIban(SouscriptionsTestMixin, TransactionCase):
    ...
    def test_iban_spaces_and_case_normalization(self):
        ...
        for iban_format in iban_formats:
            self.assertTrue(demande._validate_iban(iban_format), f'IBAN format should be valid: {iban_format}')
        ...
        for case_format in case_formats:
            self.assertTrue(demande._validate_iban(case_format), f'IBAN case should be valid: {case_format}')
