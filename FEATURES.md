...
- REQ-FAC-16 ✅ Régularisation (solde) en brouillon : modèle propre, lignes typées grille × cadran (sous-période, Σ écart, prix), candidats (facturé + écart non nul + verdict connu + non soldé en legacy + communicant), refresh mois par mois avant calcul, signalements (incalculable/absent, non communicant), mois conservé au refresh marqué « estimation locale », recalcul idempotent ; liens posés `account.move.regularisation_id` (exclusif de `periode_id`) et `souscription.releve` bi-parent (exactement un) — pas encore de génération de facture (tranche 5) ni de tampon (tranche 6) — preuve: tests/test_regularisation.py · #20, #236, ADR 0030 décisions 3-4
